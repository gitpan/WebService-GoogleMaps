...
sub generate_gd {
	...
	# We'll need to track which adjacent tile requested that the tile get added to the queue, so we know which wall we gmap boundary we hit if we hit one.
	# Start with adding the tile at the center of the viewport to the queue
...
WebService::GoogleMaps - Retrieves maps from http://maps.google.com/
...
Karl Lohner, C<< <karllohner+googlemaps@thespill.com> >>
